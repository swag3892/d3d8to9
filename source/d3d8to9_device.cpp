...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::Present(const RECT *pSourceRect, const RECT *pDestRect, HWND hDestWindowOverride, const RGNDATA *pDirtyRegion)
{
	...
	// BF1942 can call Present while a scene is still active during loading screens.
	// D3D8 tolerated this more loosely, while D3D9 rejects it with D3DERR_INVALIDCALL.
	// End the active scene before Present, then ignore source/destination rectangles.
	...
	// Some setups reject the window override after Reset. Retry without it.
	if (FAILED(hr) && hDestWindowOverride != nullptr)
	{
		hr = ProxyInterface->Present(nullptr, nullptr, nullptr, nullptr);
	}

#ifndef D3D8TO9NOLOG
	if (FAILED(hr))
	{
		LOG << "BF1942 compat: Present failed. Error code "
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::BeginScene()
{
	...
	if (FAILED(hr))
	{
		LOG << "BF1942 compat: BeginScene failed. Error code "
			<< std::hex << hr << std::dec << "!" << std::endl;
	}
#endif
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::EndScene()
{
	...
	if (FAILED(hr))
	{
		LOG << "BF1942 compat: EndScene failed. Error code "
			<< std::hex << hr << std::dec << "!" << std::endl;
	}
#endif
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::CreateVertexShader(const DWORD *pDeclaration, const DWORD *pFunction, DWORD *pHandle, DWORD Usage)
{
	...
	*pHandle = 0;
	...
	if (pFunction != nullptr)
	{
		...
		// BF1942 compatibility:
		// Do NOT rewrite partial oPos writes like "add oPos.xy, ..." into full vector writes.
		// That corrupts loading/UI billboard shaders. Instead, initialize oPos once, then keep
		// the original partial writes intact.
		if (std::regex_search(SourceCode, std::regex("    ([a-z0-9_]+) oPos\\.")) &&
			!std::regex_search(SourceCode, std::regex("    ([a-z0-9_]+) oPos,")) &&
			InstructionCount < 128)
		{
			const size_t FirstPartialOPos = SourceCode.find(" oPos.");
			const size_t LineStart = SourceCode.rfind('\n', FirstPartialOPos);

			if (LineStart != std::string::npos)
			{
				SourceCode.insert(LineStart + 1, "    mov oPos, v0 /* BF1942 compat: initialize full oPos before partial writes */\n");
				++InstructionCount;
			}
		}
#ifndef D3D8TO9NOLOG
